...
STATUS UART0_Init(UARTn uartn,uint32_t BaudRate)
{      
  //引脚初始化
  ...
  USCIX[uartn]->SYNC = BIT_RESET;                             //同步模式
  USCIX[uartn]->MST = BIT_SET;                              //主机模式
  //USCIX[spin]->MODE = spistemode;                      //片选信号设置
  //USCIX[spin]->CKMODE = spiclkmode;                    //时钟线模式设置
  //USCIX[spin]->MSB = firstbit;                         //发送首位设置
  USCIX[uartn]->SPB = BIT_SET;    /* Async. Mode: Stop Bits  0:one / 1: two */  
  USCIX[uartn]->UC_7BIT = BIT_RESET;   /* Async. Mode: Data Bits  0:8-bits / 1:7-bits */  
  USCIX[uartn]->MSB = BIT_SET;    /* Async. Mode: MSB first  0:LSB / 1:MSB */
  USCIX[uartn]->PAR  = BIT_SET;    /* Async. Mode: Parity     0:odd / 1:even */  
  USCIX[uartn]->PEN  = BIT_SET;    /* Async. Mode: Parity enable */
